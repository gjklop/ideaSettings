...
<html>
    <head>
        ...
        <script src="${"$"}{jsPath}/${moduleName}/${tableInfo.lowerCamelCase}.js" type="text/javascript" charset="UTF-8"></script>
    </head>
    ...
    <#list tableInfo.fieldInfos as fieldInfo>
        <#if !fieldInfo.primaryKey && !fieldInfo.isMultiLineType && fieldInfo.lowerColName?index_of("_id") == -1 && fieldInfo.lowerCamelCase != "recordStatus" && fieldInfo.lowerCamelCase != "createdBy" && fieldInfo.lowerCamelCase != "creationDate" && fieldInfo.lowerCamelCase != "lastUpdatedBy" && fieldInfo.lowerCamelCase != "lastUpdateDate">
            ...
            <#if loopCount == 3>
                <#break>
                    ...
                    <a href="javascript:void(0);" onclick="resetFormData('${tableInfo.lowerCamelCase}SearchForm');" class="easyui-linkbutton" data-options="iconCls:'icon-clear'">清空搜索条件</a>
                </form>
            </div>
        ...
        <!-- 新增或修改弹出窗口 -->
        <div id="${tableInfo.lowerCamelCase}AddOrModifyWindow" class="easyui-window" data-options="iconCls:'icon-add',modal:true, closed:true, resizable:false,minimizable:false,maximizable:false,collapsible:false,maximized:false" style="width:520px;height:300px;padding:2;">
